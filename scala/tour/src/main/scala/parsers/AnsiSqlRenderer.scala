...
object AnsiSqlRenderer {
  ...
  def expandFrom(query: Query): String = "from %s".format(query.from.table)

  def expandWhere(query: Query): Option[String] = {
    if (query.where.isEmpty || query.where.get.clauses.isEmpty)
      None
    else
      Option("where %s".format(query.where.get.clauses.map(expandClause).mkString(" ")))
  }
  ...
  def expandClause(clause: Clause): String = clause match {
    ...
    case in: In => "%s in (%s)".format(in.field, in.values.map(quote).mkString(","))
  ...
  def quote(value: String): String = "'%s'".format(escape(value))
