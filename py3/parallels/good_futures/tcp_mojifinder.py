...
from parallels.good_futures.charindex import InvertedIndex, format_results
...
async def supervisor(index: InvertedIndex, host: str, port: int):
    ...
    sockets = cast(tuple[TransportSocket, ...], server.sockets)
    addr = sockets[0].getsockname()
    print(f'Serving on {addr}. Hit Ctrl+C to stop.')
    await server.serve_forever()


def main(host: str = '127.0.0.1', port_arg: str = '2223'):
    port = int(port_arg)
